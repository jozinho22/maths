...
const EDL1CoefficientConstantExempleEqu1 = () => {
    ...
    var EDL1EK = "\\(  K(x) = \\int^x t^2. e^{2t}.dt \\)";
    ...
    var EDL1EK3 = "\\( K(x) = \\frac{1}{2}\\left[t^2.e^{2t}\\right]^x - \\left(\\frac{1}{2} \\left[t.e^{2t}\\right]^x -\\frac{1}{2}\\int^x e^{2t}.dt \\right) \\)";
    ...
    var EDL1EK6 = "\\( K(x) = e^{2x} \\left( \\frac{x^2}{2} - \\frac{x}{2} + \\frac{1}{4} \\right) \\)";

    var EDL1EY1 = "\\(  y_1(x) = e^{2x} \\left( \\frac{x^2}{2} - \\frac{x}{2} + \\frac{1}{4} \\right) .e^{-2x} \\)";
    var EDL1EY1bis = "\\(  y_1(x) =  \\frac{x^2}{2} - \\frac{x}{2} + \\frac{1}{4} \\)";
    ...
    return  <>
                ...
                <p>Réunissons à présent les membres de droite de <MathJaxInline toShow={"$ (2) $"} /> et de <MathJaxInline toShow={"$ (\\tilde{E_1}) $"} /> : </p>
                ...
                <p>Car il est plus facile de trouver une primitive de <MathJaxInline toShow={"$ x \\longmapsto e^{ax} $"} /> que la plupart des fonctions, et par ailleurs en dérivant un polynôme on obtient une expression de plus en plus simplifiée qui finit par être une constante.</p>
                <p>Soit ici, on aura :</p>
                <MathJaxDisplay toShow={EDL1EKintParPart2} demo />
                <p>Nous partons de cette équation : </p>
                <MathJaxDisplay toShow={EDL1EK} demo />
                <p>De même, ici on préferera prendre <MathJaxInline toShow={"$ u'(t) = e^{2t} $"} /> :</p>
                ...
                <Title2 title={"2-c) Solution particulière y1"} />
