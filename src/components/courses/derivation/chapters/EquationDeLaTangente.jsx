...
const EquationDeLaTangente = () => {
    ...
    return  <>
                <p>
                    Prenons l'exemple de <Link url={`${PagesConstants.PREFIX}${PagesConstants.COURS}${CoursesConstants.DERIVATION}/derivee-de-x-au-carre`} external>la fonction carrée</Link>, définie pour tout <MathJaxInline toShow={"$x \\in \\mathbb{R}$"} />.
                </p>
                ...
                <p>
                    Puisque c'est une droite, nous savons que son équation est de type (voir chapitre sur <Link url={`${PagesConstants.PREFIX}${PagesConstants.COURS}${CoursesConstants.EQ_1_DEG}`} external>les équations du premier degré</Link>) :
                </p>
                ...
                <p>Nous avons vu au chapitre sur <Link url={`${PagesConstants.PREFIX}${PagesConstants.COURS}${CoursesConstants.DERIVATION}/convexite-d-une-fonction`} external>la convexité</Link> qu'une fonction convexe a toujours ses tangentes situées en dessous de sa courbe représentative.</p>
